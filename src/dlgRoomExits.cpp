...
dlgRoomExits::dlgRoomExits(Host *pH, QWidget *pW)
    : QDialog(pW), mpHost(pH), mpEditItem(nullptr), pR(), mRoomID(), mEditColumn()
{
    setupUi(this);
}
...
void dlgRoomExits::slot_editSpecialExit(QTreeWidgetItem *pI, int column)
{
    ...
    if (mpEditItem != nullptr && (pI != mpEditItem || mEditColumn != column))
    {
        ...
        case 0:
            ...
            {
                mpEditItem->setText(0, tr("(room ID)", "Placeholder, if no room ID is set for an exit, yet. This "
                                                       "string is used in 2 places, ensure they match!"));
            }
            specialExits->closePersistentEditor(mpEditItem, mEditColumn);
            break;
        ...
        case 2:
            mpEditItem->setText(
                2, QString::number((mpEditItem->text(2).toInt() < 0)
                                       ? (-1 * mpEditItem->text(2).toInt())
                                       : mpEditItem->text(2).toInt())); // Force result to be non-negative integer
            specialExits->closePersistentEditor(mpEditItem, mEditColumn);
            break;
        ...
        case 7:
            ...
            {
                mpEditItem->setText(7, tr("(command or Lua script)",
                                          "Placeholder, if a special exit has no code given, yet. This string is also "
                                          "used programmatically - ensure all five instances are the same"));
            }
            specialExits->closePersistentEditor(mpEditItem, mEditColumn);
        ...
        if (pExitToRoom)
        {
            ...
            {
                mpEditItem->setToolTip(
                    0, QStringLiteral("<html><head/><body><p>%1</p><p>%2</p></body></html>")
                           .arg(tr(R"(Exit to "%1".)").arg(pExitToRoom->name),
                                tr("<b>Room</b> Weight of destination: %1.",
                                   "Bold HTML tags are used to emphasis that the value is destination room's weight "
                                   "whether overridden by a non-zero exit weight here or not.")
        ...
        else
        {
            ...
            mpEditItem->setToolTip(0, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                                          .arg(tr("Entered number is invalid, set the number of the room that this "
                                                  "special exit leads to, will turn blue for "
                                                  "a valid number; if left like this, this exit will be deleted when "
                                                  "&lt;i&gt;save&lt;/i&gt; is clicked.")));
        }

        mpEditItem = nullptr; // This will cause a new PE to be opened, it will also be zeroed on the first time this
                              // funciton is called
        mEditColumn = -1;
    }
    ...
    //    qDebug()<<"A Special Exit is been edited, it has the command:" << pI->text(7) <<"and the editing is on column
    //    "<<column;
    switch (column)
    {
    case 2:
        pI->setText(2, QString::number((pI->text(2).toInt() < 0)
                                           ? (-1 * pI->text(2).toInt())
                                           : pI->text(2).toInt())); // Force result to be non-negative
        break;
...
void dlgRoomExits::slot_addSpecialExit()
{
    ...
    pI->setText(0, tr("(room ID)", "Placeholder, if no room ID is set for an exit, yet. This string is used in 2 "
                                   "places, ensure they match!")); // Exit RoomID
    pI->setForeground(0, QColor(Qt::red));
    pI->setToolTip(0, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Set the number of the room that this special exit leads to, will turn blue for a "
                                  "valid number; if left like "
    ...
    pI->setToolTip(2, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Set to a positive value to override the default (Room) Weight for using this Exit "
                                  "route, zero value assigns the default.")));
    ...
    pI->setToolTip(4, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Green (Open) door symbol would be drawn on a custom exit line for this exit on 2D "
                                  "Map (but not currently).")));
    pI->setCheckState(5, Qt::Unchecked); // Doortype: closed
    pI->setToolTip(5, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Orange (Closed) door symbol would be drawn on a custom exit line for this exit on "
                                  "2D Map (but not currently).")));
    pI->setCheckState(6, Qt::Unchecked); // Doortype: locked
    pI->setToolTip(6, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Red (Locked) door symbol would be drawn on a custom exit line for this exit on 2D "
                                  "Map (but not currently).")));
    pI->setText(7, tr("(command or Lua script)",
                      "Placeholder, if a special exit has no code given, yet. This string is also used "
                      "programmatically - ensure all five instances are the same")); // Exit command
...
void dlgRoomExits::save()
{
    ...
    if (nw->isEnabled() && !nw->text().isEmpty() && mpHost->mpMap->mpRoomDB->getRoom(nw->text().toInt()) != nullptr)
    {
        ...
        { // And ensure that stub exit is cleared if set
            pR->setExitStub(dirCode, false);
        }
        if (weight_nw->value())
        { // And store any weighing specifed
    ...
    dirCode = DIR_DOWN;
    if (down->isEnabled() && !down->text().isEmpty() &&
        mpHost->mpMap->mpRoomDB->getRoom(down->text().toInt()) != nullptr)
    {
...
void dlgRoomExits::slot_nw_textEdited(const QString &text)
{
        ...
        if (exitToRoom->name.trimmed().length())
        {
            nw->setToolTip(QStringLiteral("<html><head/><body><p>%1</p><p>%2</p></body></html>")
                               .arg(tr(R"(Exit to "%1".)").arg(exitToRoom->name),
                                    tr("<b>Room</b> Weight of destination: %1.",
                                       "Bold HTML tags are used to emphasis that the value is destination room's "
                                       "weight whether overridden by a non-zero exit weight here or not.")
    ...
    else if (!text.isEmpty())
    {
        ...
        nw->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Entered number is invalid, set the number of the room northwest of this one, will "
                                   "turn blue for a valid number.")));
    ...
    else
    {
        ...
        nw->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Set the number of the room northwest of this one, will be blue for a valid number "
                                   "or red for invalid.")));
        stub_nw->setEnabled(true);
        noroute_nw->setEnabled(false);
        weight_nw->setEnabled(false);
...
void dlgRoomExits::slot_n_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        n->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Entered number is invalid, set the number of the room north of this one, will turn "
                                  "blue for a valid number.")));
    ...
    else
    {
        ...
        n->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Set the number of the room north of this one, will be blue for a valid number or "
                                  "red for invalid.")));
        stub_n->setEnabled(true);
        noroute_n->setEnabled(false);
        weight_n->setEnabled(false);
...
void dlgRoomExits::slot_ne_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        ne->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Entered number is invalid, set the number of the room northeast of this one, will "
                                   "turn blue for a valid number.")));
    ...
    else
    {
        ...
        ne->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Set the number of the room northeast of this one, will be blue for a valid number "
                                   "or red for invalid.")));
        stub_ne->setEnabled(true);
        noroute_ne->setEnabled(false);
        weight_ne->setEnabled(false);
...
void dlgRoomExits::slot_up_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        up->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Entered number is invalid, set the number of the room up from this one, will turn "
                                   "blue for a valid number.")));
    ...
    else
    {
        ...
        up->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Set the number of the room up from this one, will be blue for a valid number or "
                                   "red for invalid.")));
        stub_up->setEnabled(true);
        noroute_up->setEnabled(false);
        weight_up->setEnabled(false);
...
void dlgRoomExits::slot_w_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        w->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Entered number is invalid, set the number of the room west of this one, will turn "
                                  "blue for a valid number.")));
    ...
    else
    {
        ...
        w->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Set the number of the room west of this one, will be blue for a valid number or red "
                                  "for invalid.")));
        stub_w->setEnabled(true);
        noroute_w->setEnabled(false);
        weight_w->setEnabled(false);
...
void dlgRoomExits::slot_e_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        e->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Entered number is invalid, set the number of the room east of this one, will turn "
                                  "blue for a valid number.")));
    ...
    else
    {
        ...
        e->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Set the number of the room east of this one, will be blue for a valid number or red "
                                  "for invalid.")));
        stub_e->setEnabled(true);
        noroute_e->setEnabled(false);
        weight_e->setEnabled(false);
...
void dlgRoomExits::slot_down_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        down->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                             .arg(tr("Entered number is invalid, set the number of the room down from this one, will "
                                     "turn blue for a valid number.")));
    ...
    else
    {
        ...
        down->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                             .arg(tr("Set the number of the room down from this one, will be blue for a valid number "
                                     "or red for invalid.")));
        stub_down->setEnabled(true);
        noroute_down->setEnabled(false);
        weight_down->setEnabled(false);
...
void dlgRoomExits::slot_sw_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        sw->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Entered number is invalid, set the number of the room southwest of this one, will "
                                   "turn blue for a valid number.")));
    ...
    else
    {
        ...
        sw->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Set the number of the room southwest of this one, will be blue for a valid number "
                                   "or red for invalid.")));
        stub_sw->setEnabled(true);
        noroute_sw->setEnabled(false);
        weight_sw->setEnabled(false);
...
void dlgRoomExits::slot_s_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        s->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Entered number is invalid, set the number of the room south of this one, will turn "
                                  "blue for a valid number.")));
    ...
    else
    {
        ...
        s->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                          .arg(tr("Set the number of the room south of this one, will be blue for a valid number or "
                                  "red for invalid.")));
        stub_s->setEnabled(true);
        noroute_s->setEnabled(false);
        weight_s->setEnabled(false);
...
void dlgRoomExits::slot_se_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        se->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Entered number is invalid, set the number of the room southeast of this one, will "
                                   "turn blue for a valid number.")));
    ...
    else
    {
        ...
        se->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Set the number of the room southeast of this one, will be blue for a valid number "
                                   "or red for invalid.")));
        stub_se->setEnabled(true);
        noroute_se->setEnabled(false);
        weight_se->setEnabled(false);
...
void dlgRoomExits::slot_in_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        in->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Entered number is invalid, set the number of the room in from this one, will turn "
                                   "blue for a valid number.")));
    ...
    else
    {
        ...
        in->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                           .arg(tr("Set the number of the room in from this one, will be blue for a valid number or "
                                   "red for invalid.")));
        stub_in->setEnabled(true);
        noroute_in->setEnabled(false);
        weight_in->setEnabled(false);
...
void dlgRoomExits::slot_out_textEdited(const QString &text)
{
    ...
    else if (text.size() > 0)
    {
        ...
        out->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                            .arg(tr("Entered number is invalid, set the number of the room out from this one, will "
                                    "turn blue for a valid number.")));
    ...
    else
    {
        ...
        out->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                            .arg(tr("Set the number of the room out from this one, will be blue for a valid number or "
                                    "red for invalid.")));
        stub_out->setEnabled(true);
        noroute_out->setEnabled(false);
        weight_out->setEnabled(false);
...
void dlgRoomExits::initExit(int roomId, int direction, int exitId, QLineEdit *exitLineEdit, QCheckBox *noRoute,
                            QCheckBox *stub, QRadioButton *none, QRadioButton *open, QRadioButton *closed,
                            QRadioButton *locked, QSpinBox *weight)
{
    ...
    {
    case DIR_NORTHWEST:
        doorAndWeightText = QStringLiteral("nw");
        exitText = tr("northwest");
        break;
    case DIR_NORTH:
        doorAndWeightText = QStringLiteral("n");
        exitText = tr("north");
        break;
    case DIR_NORTHEAST:
        doorAndWeightText = QStringLiteral("ne");
        exitText = tr("northeast");
        break;
    case DIR_UP:
        doorAndWeightText = QStringLiteral("up");
        exitText = tr("up");
        break;
    case DIR_WEST:
        doorAndWeightText = QStringLiteral("w");
        exitText = tr("west");
        break;
    case DIR_EAST:
        doorAndWeightText = QStringLiteral("e");
        exitText = tr("east");
        break;
    case DIR_DOWN:
        doorAndWeightText = QStringLiteral("down");
        exitText = tr("down");
        break;
    case DIR_SOUTHWEST:
        doorAndWeightText = QStringLiteral("sw");
        exitText = tr("southwest");
        break;
    case DIR_SOUTH:
        doorAndWeightText = QStringLiteral("s");
        exitText = tr("south");
        break;
    case DIR_SOUTHEAST:
        doorAndWeightText = QStringLiteral("se");
        exitText = tr("southeast");
        break;
    case DIR_IN:
        doorAndWeightText = QStringLiteral("in");
        exitText = tr("in");
        break;
    case DIR_OUT:
        doorAndWeightText = QStringLiteral("out");
        exitText = tr("out");
        break;
    default:
        Q_UNREACHABLE();
    ...
    case 3:
        ...
        break;
    default:
        qWarning() << "dlgRoomExits::initExit(...) in room id(" << roomId << ") unexpected doors[" << doorAndWeightText
                   << "] value:" << pR->getDoor(doorAndWeightText) << "found for room!";
    }
    ...
    if (exitId > 0)
    {
        ...
        if (!pExitR)
        {
            ...
            qWarning() << "dlgRoomExits::initExit(...): Warning: missing exit to" << exitId << "in direction"
                       << exitText << ", resetting exit.";
            exitId = -1;
        }
    }

    if (exitId > 0 && pExitR)
    {                                                   // Does this exit point anywhere
        ...
        {
            exitLineEdit->setToolTip(
                QStringLiteral("<html><head/><body><p>%1</p><p>%2</p></body></html>")
                    .arg(tr(R"(Exit to "%1".)").arg(pExitR->name),
                         tr("<b>Room</b> Weight of destination: %1.",
                            "Bold HTML tags are used to emphasis that the value is destination room's weight whether "
                            "overridden by a non-zero exit weight here or not.")
    ...
    {                                     // No exit is set on initialisation
        ...
        else
        {
            ...
            exitLineEdit->setToolTip(QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                                         .arg(tr("Set the number of the room %1 of this one, will be blue for a valid "
                                                 "number or red for invalid.")
                                                  .arg(exitText)));
...
void dlgRoomExits::init(int id)
{
    ...
    initExit(id, DIR_NORTHWEST, pR->getExit(DIR_NORTHWEST), nw, noroute_nw, stub_nw, doortype_none_nw, doortype_open_nw,
             doortype_closed_nw, doortype_locked_nw, weight_nw);

    initExit(id, DIR_NORTH, pR->getExit(DIR_NORTH), n, noroute_n, stub_n, doortype_none_n, doortype_open_n,
             doortype_closed_n, doortype_locked_n, weight_n);

    initExit(id, DIR_NORTHEAST, pR->getExit(DIR_NORTHEAST), ne, noroute_ne, stub_ne, doortype_none_ne, doortype_open_ne,
             doortype_closed_ne, doortype_locked_ne, weight_ne);

    initExit(id, DIR_UP, pR->getExit(DIR_UP), up, noroute_up, stub_up, doortype_none_up, doortype_open_up,
             doortype_closed_up, doortype_locked_up, weight_up);

    initExit(id, DIR_WEST, pR->getExit(DIR_WEST), w, noroute_w, stub_w, doortype_none_w, doortype_open_w,
             doortype_closed_w, doortype_locked_w, weight_w);

    initExit(id, DIR_EAST, pR->getExit(DIR_EAST), e, noroute_e, stub_e, doortype_none_e, doortype_open_e,
             doortype_closed_e, doortype_locked_e, weight_e);

    initExit(id, DIR_DOWN, pR->getExit(DIR_DOWN), down, noroute_down, stub_down, doortype_none_down, doortype_open_down,
             doortype_closed_down, doortype_locked_down, weight_down);

    initExit(id, DIR_SOUTHWEST, pR->getExit(DIR_SOUTHWEST), sw, noroute_sw, stub_sw, doortype_none_sw, doortype_open_sw,
             doortype_closed_sw, doortype_locked_sw, weight_sw);

    initExit(id, DIR_SOUTH, pR->getExit(DIR_SOUTH), s, noroute_s, stub_s, doortype_none_s, doortype_open_s,
             doortype_closed_s, doortype_locked_s, weight_s);

    initExit(id, DIR_SOUTHEAST, pR->getExit(DIR_SOUTHEAST), se, noroute_se, stub_se, doortype_none_se, doortype_open_se,
             doortype_closed_se, doortype_locked_se, weight_se);

    initExit(id, DIR_IN, pR->getExit(DIR_IN), in, noroute_in, stub_in, doortype_none_in, doortype_open_in,
             doortype_closed_in, doortype_locked_in, weight_in);

    initExit(id, DIR_OUT, pR->getExit(DIR_OUT), out, noroute_out, stub_out, doortype_none_out, doortype_open_out,
             doortype_closed_out, doortype_locked_out, weight_out);
    ...
    while (it.hasNext())
    {
        ...
        if (pExitToRoom)
        {
            ...
            if (!pExitToRoom->name.isEmpty())
            {
                pI->setToolTip(0, QStringLiteral("<html><head/><body><p>%1</p><p>%2</p></body></html>")
                                      .arg(tr(R"(Exit to "%1".)").arg(pExitToRoom->name),
                                           tr("<b>Room</b> Weight of destination: %1.",
                                              "Bold HTML tags are used to emphasis that the value is destination "
                                              "room's weight whether overridden by a non-zero exit weight here or not.")
                                               .arg(pExitToRoom->getWeight())));
            }
            else
            {
        ...
        else
        {
            pI->setForeground(0, QColor(Qt::red));
            pI->setToolTip(0, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                                  .arg(tr("Room Id is invalid, set the number of the room that this special exit leads "
                                          "to, will turn blue for a valid number.")));
        }
        ...
        pI->setToolTip(
            1, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                   .arg(tr("Prevent a route being created via this exit, equivalent to an infinite exit weight.")));
        ...
        pI->setTextAlignment(2, Qt::AlignRight);
        pI->setToolTip(2, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                              .arg(tr("Set to a positive value to override the default (Room) Weight for using this "
                                      "Exit route, zero value assigns the default.")));
        ...
        pI->setToolTip(
            3, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                   .arg(tr("No door symbol is drawn on 2D Map for this exit (only functional choice currently).")));
        pI->setCheckState(4, Qt::Unchecked);
        pI->setTextAlignment(4, Qt::AlignCenter);
        pI->setToolTip(4, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                              .arg(tr("Green (Open) door symbol would be drawn on a custom exit line for this exit on "
                                      "2D Map (but not currently).")));
        pI->setCheckState(5, Qt::Unchecked);
        pI->setTextAlignment(5, Qt::AlignCenter);
        pI->setToolTip(5, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                              .arg(tr("Orange (Closed) door symbol would be drawn on a custom exit line for this exit "
                                      "on 2D Map (but not currently).")));
        pI->setTextAlignment(6, Qt::AlignCenter);
        pI->setToolTip(6, QStringLiteral("<html><head/><body><p>%1</p></body></html>")
                              .arg(tr("Red (Locked) door symbol would be drawn on a custom exit line for this exit on "
                                      "2D Map (but not currently).")));
        pI->setCheckState(6, Qt::Unchecked);
        {
            ...
            case 3:
                ...
                break;
            default:
                qDebug() << "dlgRoomExits::init(" << id << ") unexpected (other exit) doors[" << dir
                         << "] value:" << pR->doors[dir] << " found!";
            }
...
void dlgRoomExits::slot_checkModified()
{
    ...
    if (!isModified)
    {
        ...
        {
            QTreeWidgetItem *pI = specialExits->topLevelItem(i);
            /*            qDebug("dlgRoomExits::slot_checkModified() considering specialExit (item %i, pass 1) to:%i,
             * command:%s", i, pI->text(0).toInt(), qPrintable(pI->text(7)));
             */
            if (pI->text(7) == tr("(command or Lua script)",
                                  "Placeholder, if a special exit has no code given, yet. This string is also used "
                                  "programmatically - ensure all five instances are the same") ||
                pI->text(0).toInt() <= 0)
        ...
        else
        {
            if (originalCount)
            {
                ...
                {
                    QTreeWidgetItem *pI = specialExits->topLevelItem(i);
                    /*                    qDebug("dlgRoomExits::slot_checkModified() considering specialExit (item %i,
                     * pass 2) to:%i, command:%s", i, pI->text(0).toInt(), qPrintable(pI->text(7)));
                     */
                    if (pI->text(7) == tr("(command or Lua script)",
                                          "Placeholder, if a special exit has no code given, yet. This string is also "
                                          "used programmatically - ensure all five instances are the same") ||
                    ...
                    currentExit.door =
                        pI->checkState(6) == Qt::Checked
                            ? 3
                            : pI->checkState(5) == Qt::Checked ? 2 : pI->checkState(4) == Qt::Checked ? 1 : 0;
                    currentExit.weight = pI->text(2).toInt();
                    currentExit.hasStub = false;
                    auto exit = foundMap.value(currentCmd);
                    if (exit && exit->destination == currentExit.destination && exit->door == currentExit.door &&
                        exit->hasNoRoute == currentExit.hasNoRoute && exit->weight == currentExit.weight)
                    {
